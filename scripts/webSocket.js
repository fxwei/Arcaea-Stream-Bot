...
function webSocket(room_id=default_ID){
...
on("INTERACT_WORD", function(e){
    ...
    if (data.data.msg_type == 2){    //个人推测，不一定准确
        console.log(time+ " " + uname + "关注了直播间");

    }else{
        console.log(/*data.cmd + " " + */time+ " " + uname + "进入直播间","misc");
    }
})
...
on("DANMU_MSG", function(e){
    ...
    console.log(/*data.cmd + " " + */time+ " " + uname + ": " + text,"danmu");
    parseText(text);
})
...
//直播间号码
// webSocket(0);
// 作者：鱼肉烧 https://www.bilibili.com/read/cv14101053 出处：bilibili